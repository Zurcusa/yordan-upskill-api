...
import * as auctionAbi from '../ABI/Auction.json';
...
import { AuctionEventInputsValidator } from '../utils/AuctionEventInputsValidator';
import {
  AUCTION_EVENTS,
  OPERATIONS,
  CONFIG_KEYS,
...
export class ContractListenerService implements OnApplicationBootstrap {
  ...
  private listenToNftEvents() {
    this.zurcusNft.on(AUCTION_EVENTS.TRANSFER, async (from, to, tokenId) => {
      try {
        ...
        throw new AuctionListenerError(this.zurcusNft.target.toString(), error);
      }
    });
  }
  ...
  private async persistOrUpdateNft(tokenId: string, owner: string) {
    try {
      ...
      throw new PersistenceError(OPERATIONS.SAVE_NFT_RECORD, error);
    }
  }
  ...
  private async persistUser(walletAddress: string) {
    try {
      ...
      throw new PersistenceError(OPERATIONS.SAVE_USER_RECORD, error);
    }
  }
  ...
  private async pruneOutdatedAuctions() {
    try {
      ...
      throw new PersistenceError(OPERATIONS.REMOVE_OLD_AUCTIONS, error);
    }
  }
}
